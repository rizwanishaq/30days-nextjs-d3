...
self.onmessage = function (e) {
  const { type, timeDomainArray, sampleRate } = e.data;

  if (type === 'time-domain') {
    if (timeDomainArray) {
      // Convert timeDomainArray to 16 kHz sample frequency using polyphase resampling
      const downsampledArray = downsampleBuffer(timeDomainArray, sampleRate, 16000);

      // Send time-domain data and sampling frequency back to the main thread
      self.postMessage({
        type: 'time-domain',
        data: convertFloat32ToInt16(downsampledArray),
